...
Before multi-check execution, the verification contract freezes each check id, covered obligations, concrete non-empty input manifest, dependency order, persistent receipt root, and one primary execution owner. Every governed source input must materialize byte-for-byte in the frozen root; tracked authority names that match OpenSpec's generated-output classifier are blocked before release. Runtime prerequisites are generated inside their owning check, while ignored local records stay outside the admitted snapshot. A metadata-free frozen root validates source content but leaves clean-branch, commit, tag, and hosted-release identity to separate live-repository publication gates. When a native audit binds logical project identity to the root basename, its owner creates a minimal temporary projection named by the unchanged `project_id` and still consumes the native result. TestMesh aggregates current immutable child receipts and never launches or reruns the child commands. Full validation runs once on a frozen integration snapshot. After a timeout, the launcher must confirm zero descendants before any result can be reused.
...
- **A frozen owner depends on ignored runtime state, admits local records, binds an empty directory shorthand, requires absent Git metadata, or mistakes a random snapshot name for project identity** -> generate runtime prerequisites inside that owner, exclude local records, bind concrete admitted-source selectors, keep VCS identity in the live publication gate, project stable logical identity from unchanged declared inputs, and replay the finite boundary family.
