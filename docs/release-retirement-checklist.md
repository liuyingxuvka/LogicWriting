...
This checklist describes the `4.0.1` corrective release. Checkboxes are coordination aids,
...
- [ ] Confirm LogicGuard, SourceGuard, and TraceGuard retain separate semantic
  ...
  `researchguard trace` from ResearchGuard `0.5.2`.
...
- [ ] Freeze `VERSION`, `pyproject.toml`, both README source badges, changelog,
  ...
  SkillGuard consumer-distribution identity on the same `4.0.1` source tree.
...
- [ ] Push the default branch, create signed/annotated `v4.0.1` as configured,
