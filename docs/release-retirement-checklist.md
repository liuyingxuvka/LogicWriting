...
This checklist describes the `2.1.2` cutover. Checkboxes are coordination aids,
...
- [ ] Confirm LogicGuard, SourceGuard, and TraceGuard retain separate semantic
  ...
  `researchguard trace` from ResearchGuard `0.1.2`.
...
- [ ] Freeze `VERSION`, `pyproject.toml`, both README source badges, changelog,
  ...
  SkillGuard consumer-distribution identity on the same `2.1.2` source tree.
...
- [ ] Push the default branch, create signed/annotated `v2.1.2` as configured,
