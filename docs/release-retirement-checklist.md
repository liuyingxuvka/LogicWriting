...
This checklist describes the `3.0.1` corrective release. Checkboxes are coordination aids,
...
- [ ] Confirm LogicGuard, SourceGuard, and TraceGuard retain separate semantic
  ...
  `researchguard trace` from ResearchGuard `0.4.5`.
...
- [ ] Freeze `VERSION`, `pyproject.toml`, both README source badges, changelog,
  ...
  SkillGuard consumer-distribution identity on the same `3.0.1` source tree.
...
- [ ] Push the default branch, create signed/annotated `v3.0.1` as configured,
