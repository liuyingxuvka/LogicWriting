...
description: Propose a new change with all artifacts generated in one step. Use when the user wants to quickly describe what they want to build and get a complete proposal with design, specs, verification contract, and tasks ready for implementation.
license: MIT
...
- specs/<capability>/spec.md (what must be true)
- design.md (how)
- verification-contract.yaml (how completion will be proven)
...
- Prompt: "Run `/opsx:apply` or ask me to implement to start working on the tasks. After tasks are complete, run `/opsx:verify` before archive."
...
- For strict schemas, create `verification-contract.yaml` before `tasks.md` so the task list is derived from concrete evidence obligations.
...
## Entrypoint Scope
Covers openspec-propose plus explicitly routed local materials; no unrelated repos, private files, external services, publication, or release claims unless requested and routed.
...
## Use When
Use when the request matches openspec-propose and needs this governed workflow, materials, checks, or handoff behavior.
## Do Not Use When
