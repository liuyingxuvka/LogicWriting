...
3. **Get apply instructions**
   ...
   This returns:
   - `contextFiles`: artifact ID -> array of concrete file paths (varies by schema - could be proposal/specs/design/verification-contract/tasks or spec/tests/implementation/docs)
   ...
   **Handle states:**
   ...
   - If `state: "all_done"`: state that implementation tasks are complete, then run or recommend `openspec verify <name>` before archive
...
4. **Read context files**
   ...
   - **spec-driven**: proposal, specs, design, verification-contract, tasks
...
7. **On completion or pause, show status**
   ...
   - If all done: run or recommend `openspec verify <name>` before archive
...
All tasks complete. Run `openspec verify <change-name>` now; archive is only appropriate after the verification report passes and remains current.
```
...
## Entrypoint Scope
Covers openspec-apply-change plus explicitly routed local materials; no unrelated repos, private files, external services, publication, or release claims unless requested and routed.
...
## Use When
Use when the request matches openspec-apply-change and needs this governed workflow, materials, checks, or handoff behavior.
## Do Not Use When
