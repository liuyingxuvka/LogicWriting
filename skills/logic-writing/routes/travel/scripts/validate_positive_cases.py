...
CASE_SCHEMA = "travel-story-planner.positive-case.v1"
...
def run_case(path: Path, repository_root: Path) -> dict[str, Any]:
    ...
    return {"case": str(path), "case_id": case.get("case_id"), "ok": not issues, "issues": issues}


def main() -> int:
    parser = argparse.ArgumentParser(description="Run current positive Travel branch cases.")
