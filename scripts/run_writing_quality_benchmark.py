...
def _writer_prompt(case: Mapping[str, Any], version: str) -> str:
    ...
    route_focus = {
        ...
        "travel-guide": (
            ...
            "有多个候选路线时，默认路线的可行条件和每条备用路线的退回条件必须分别绑定到所选路线；"
            "某条备用路线核实失败不能否决已经核实可行的默认路线，也不要用跨路线的一个‘或’或‘且’条件收束所有选择。"
        ),
...
def _composition_guidance(case: Mapping[str, Any]) -> str:
    ...
    guidance = {
        ...
        "F01": (
            ...
            "不能新增第二把钥匙，也不能把白漆直接解释为调钥匙事实；主管放行后要写清由谁实际解除锁闭并开门，"
            "不要只跳到‘工人进入’。三声船铃按事件计数：全文只写三次铃声，"
            "每个节点只写一次，不要每次再写三下而累计成九声；三次分别承载日常秩序、倒计时压力和救单后的关系变化。"
        ),
        "F02": (
            "内部场景链必须闭合为：明确林岚在锁闭仓库外→铃声由日常变成倒计时→公开账页改变主管的可行选择→"
            "现场可见的放行与解除锁闭动作让货单获救→第三次铃声事件落在弟弟的距离变化上。三次铃声是三个事件，"
            "全文总共写三次、每个节点一次，不要扩成九声。不得把当前钥匙写成必然可用，"
        ...
        "F03": (
            ...
            "不要把‘三项’扩成资料清单；保留三次船铃事件及其递进作用，不要求每次连续三声，也不要累计成九声。"
        ),
        "T01": (
        ...
        "H-T": (
            "内部路线链固定为：先在连续步行上限内选择馆B作为默认→安排到馆、馆内和返程的休息→"
            "把馆B营业、婴儿车通行和休息衔接列为出发前核实→把公交去馆A作为独立备用，单独核实班次、"
            "馆A营业和电梯维修期间的婴儿车通行→按所选路线分别决定是否留在旅馆。若馆B核实通过而公交去馆A"
            "未通过，仍可按馆B出行；只有馆B条件未通过才取消馆B，只有选择公交去馆A时其条件未通过才取消该路线。"
            "不可把两条路线合成一个全局的‘或’或‘且’退回条件。"
        ),
    }
