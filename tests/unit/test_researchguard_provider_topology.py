...
class _Distribution:
    def __init__(self, executable: Path, version: str = "0.5.1") -> None:
...
def _completed(argv, **_kwargs):
    if argv[1:] == ["--version"]:
        return SimpleNamespace(returncode=0, stdout="researchguard 0.5.1\n", stderr="")
...
def test_each_member_uses_one_researchguard_console(monkeypatch, tmp_path):
    ...
    for member_id, (member_command, primary_path) in expected.items():
        ...
        assert result["evidence"]["suite_version"] == "0.5.1"
        assert result["evidence"]["distribution_version"] == "0.5.1"
